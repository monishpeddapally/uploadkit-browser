...
import type { DecodeResult, PrepareOptions, PreparedUpload } from "./types.js";

const DEFAULT_MAX_DIMENSION = 2048;

function abortIfNeeded(signal?: AbortSignal): void {
  if (signal?.aborted) throw new DOMException("Upload preparation aborted.", "AbortError");
}

async function nativeDecode(file: File): Promise<DecodeResult> {
  if (!file.type.startsWith("image/")) throw new TypeError("Only image files are supported in v0.1.");
  try {
...
export async function prepareUpload(file: File, options: PrepareOptions = {}): Promise<PreparedUpload> {
  ...
  try {
    ...
    const dimensions = fitDimensions(
      decoded.width,
      decoded.height,
      options.maxWidth ?? DEFAULT_MAX_DIMENSION,
      options.maxHeight ?? DEFAULT_MAX_DIMENSION,
      options.preventUpscale ?? true,
    );
...
export function prepareUploads(files: ArrayLike<File> | Iterable<File>, options: PrepareOptions = {}): Promise<PreparedUpload[]> {
  return Promise.all(Array.from(files as ArrayLike<File>, (file) => prepareUpload(file, options)));
}
